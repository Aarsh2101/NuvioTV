...
internal fun PlayerRuntimeController.initializePlayer(
    ...
    scope.launch {
        try {
            ...
            // Progressive MP4 playback without parallel connections stays on the
            // normal Media3 path; only the explicit parallel mode allocates chunk
            // buffering overhead.
            val useChunkSessionSource = parallelActive && !isHlsStream && !isDashStream

            val parallelOverheadMb = if (useChunkSessionSource) {
                val connCount = playerSettings.parallelConnectionCount
